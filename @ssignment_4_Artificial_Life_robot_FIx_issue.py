...
class Artificial_Life_roboot(Robot):
    ...
    def update(self):
        ...
        if self.energy < 0 :
            ...
            self.iteration = 0
            Artificial_Life_roboot.NumberOfDeth +=1
            print (f"Dead Robot position: {self.pos}")
            print (f"Total Death count {Artificial_Life_roboot.NumberOfDeth}, Iteration- ", self._sm.iteration)
            write_death(self._sm.iteration,Artificial_Life_roboot.NumberOfDeth)
        ...
        if self.iteration == 5000 :
            ...
            Artificial_Life_roboot.NumberOfDeth+=1
            print (f"Total Death count {Artificial_Life_roboot.NumberOfDeth}, Iteration- * old die ** ", self._sm.iteration)
            write_death(self._sm.iteration,Artificial_Life_roboot.NumberOfDeth)
...
def write_death(deathiteration,NumberOfDeth):
    with open("Death-iteration.csv","a") as f:
        writer = csv.writer(f, lineterminator="\n")
        death_list = list()
        death_list.append(NumberOfDeth)
        death_list.append(deathiteration)
        writer.writerow(death_list)
        print("Writing - ",death_list)
        print("deathiteration - ",deathiteration)
